    ...
    public class Node
    {
        ...
        */
        ...
        public void UpdateGradient(Layer curLayer, double target = 0d)
        {
            ...
            {
                gradient = Cost.CROSS_ENTROPY.CostFunctionIterationDerivative(target, value) * Activations.SoftMax.Derivative(curLayer,value);
            } else
            {
                gradient = outputConnections.Sum(con => (con.nodeOut.gradient * con.weight)) * Activations.Tanh.Derivative(value);
            }
        }
